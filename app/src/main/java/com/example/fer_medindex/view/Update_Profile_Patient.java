...
import static com.example.fer_medindex.view.ProfilePatient.PATIENT_ID;
...
public class Update_Profile_Patient extends AppCompatActivity {
...
//        });
    ...
    private void updateProfile(FirebaseUser firebaseUser) {
        ...
        if (TextUtils.isEmpty(textFullName)) {
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng nhập đầy đủ họ và tên của bạn", Toast.LENGTH_LONG).show();
            editTextRegisterFullName.setError("Bắt buộc nhập họ và tên");
            editTextRegisterFullName.requestFocus();// yeu cau nhap lai
        } else if (TextUtils.isEmpty((textEmail))) {
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng nhập email của bạn", Toast.LENGTH_LONG).show();
            ...
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng nhập lại email của bạn", Toast.LENGTH_LONG).show();
            editTextRegisterEmail.setError(" email không hợp lệ");
            editTextRegisterEmail.requestFocus();
        } else if (TextUtils.isEmpty(textDoB)) {
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng nhập ngày sinh của bạn", Toast.LENGTH_LONG).show();
            editTextRegisterDoB.setError("Bắt buộc nhập ngày sinh");
            editTextRegisterDoB.requestFocus();
        } else if (radioGroupRegisterGender.getCheckedRadioButtonId() == -1) {
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng chọn giới tính của bạn", Toast.LENGTH_LONG).show();
            radioButtonRegisterGenderSelected.setError("Bắt buộc phải chọn giới tính");
            radioButtonRegisterGenderSelected.requestFocus();
        } else if (TextUtils.isEmpty(textMobile)) {
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng nhập số điện thoại của bạn ", Toast.LENGTH_LONG).show();
            ...
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng nhập CMND của bạn", Toast.LENGTH_LONG).show();
            editTextRegisterEmail.setError("Bắt buộc nhập CMND");
            editTextRegisterEmail.requestFocus();
        } else if (TextUtils.isEmpty((textAddress))) {
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng nhập địa chỉ của bạn", Toast.LENGTH_LONG).show();
            ...
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng chọn tình trạng hiện tại của bạn", Toast.LENGTH_LONG).show();
            editTextRegisterEmail.setError("Bắt buộc chọn tình trạng hiện tại");
            editTextRegisterEmail.requestFocus();
        } else if (!checkboxcamket.isChecked()) {
            Toast.makeText(Update_Profile_Patient.this, "Vui lòng chọn cam kết những thông tin khai là đúng sự thật", Toast.LENGTH_LONG).show();
            ...
            referenceProfile.child(writerPatientDetails.getPatientId()).setValue(writerPatientDetails).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
...
//                        finalFirebaseUser.updateProfile(profileUpdatesImage);
                        ...
                        try{
                            ...
                            Toast.makeText(Update_Profile_Patient.this, e.getMessage(),Toast.LENGTH_LONG).show();
                        }
                    }
                    progressBar.setVisibility(View.GONE);
                }
        ...
        referenceProfile.child(userIDofRegistered).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if(readWritePatientDetails!=null){
                    textFullName = readWritePatientDetails.getFullname();
                    textEmail = readWritePatientDetails.getEmail();
                    textDoB = readWritePatientDetails.getNgaysinh();
                    textMobile = readWritePatientDetails.getSodienthoai();
                    textCMND = readWritePatientDetails.getCMND();
                    textAddress = readWritePatientDetails.getDiachi();
                    textGender = readWritePatientDetails.getGioitinh();
                    texttrangthai = readWritePatientDetails.getTrangthai();

                    editTextRegisterFullName.setText(textFullName);
                    editTextRegisterDoB.setText(textDoB);
                    editTextRegisterMobile.setText(textMobile);
                    editTextRegisterEmail.setText(textEmail);
        ...
        builder.setSingleChoiceItems(items, -1, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case 0:
                        dialog.dismiss();
                        ActivityCompat.requestPermissions(Update_Profile_Patient.this, new String[]{Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE}, REQUEST_CODE_CAMERA);
                        break;
                    case 1:
                        dialog.dismiss();
                        ActivityCompat.requestPermissions(Update_Profile_Patient.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, SELECT_PICTURE);
                        break;
                }
            }
        });
    ...
    private void xuLyAnh(Bitmap bitmap) {
        ...
        try {
            Model model = Model.newInstance(Update_Profile_Patient.this);
